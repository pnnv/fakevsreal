<!-- <script>
	...
	async function analyzeAccount() {
	  ...
	  try {
		...
		results.set({
		  ...
		  isFake: data.is_fake,
		  confidence: parseFloat((data.fake_probability * 100).toFixed(1)),
		  profileInfo: data.profile_info // Store profile info in the results
		});
	  } catch (err) {
		console.error(err);
		error.set('An error occurred while analyzing the account');
  ...
  <div class="min-h-screen px-4 py-8">
	<div class="max-w-3xl mx-auto">
	  ...
	  <div transition:fade class="bg-white rounded-lg shadow-lg p-6">
		<div class="flex items-center justify-between mb-4">
		  <h3 class="text-xl font-semibold">Analysis Results for {$results.username}:</h3>
		</div>
  
		{#if $results.profileInfo}
		<div class="profile-info mb-4">
		  {#if $results.profileInfo.profile_pic_url}
			<img src={$results.profileInfo.profile_pic_url} alt="Profile Picture" class="profile-pic" /> 
		  {/if}
		  <div class="profile-details">
			<p><strong>Full Name:</strong> {$results.profileInfo.full_name}</p>
			<p><strong>Bio:</strong> {$results.profileInfo.biography}</p>
			<p><strong>Posts:</strong> {$results.profileInfo.num_posts}</p>
			<p><strong>Followers:</strong> {$results.profileInfo.num_followers}</p>
			<p><strong>Following:</strong> {$results.profileInfo.num_follows}</p>
			{#if $results.profileInfo.external_url}
			  <a href={$results.profileInfo.external_url} target="_blank">External URL</a>
			{/if}
		  </div>
		</div>
		{/if}
